...
const IdeaBoard: React.FC<IdeaBoardProps> = ({ event }) => {
  ...
  const confirmEditIdea = async () => {
    ...
    try {
      ...
      const updatedIdea: Idea = (await response.json()) as Idea;
      setIdeas((prevIdeas) =>
                  ...
                  <CardHeader>
                    ...
                    <CardDescription className="truncate">
                      ...
                      <span className="ml-2 text-xs">
                        {idea.editedAt
                          ? `Edited ${formatDistanceToNow(new Date(idea.editedAt), {
                              addSuffix: true,
                            })}`
        ...
        return (
          <Dialog open={true} onOpenChange={() => setSelectedIdeaId(null)}>
            <DialogContent className="justify-center max-h-screen w-80 rounded-md border-border border bg-background text-foreground md:w-full">
              ...
              <DialogFooter>
                {isEventAuthor && (
                  <Button variant="default" onClick={() => openConfirmationDialog(idea.id)}>
                    Confirm this idea!
                  </Button>
                )}
              </DialogFooter>
